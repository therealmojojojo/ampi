...
def main():
    ...
    if (not setup_logging(console_log_output="stdout", console_log_level="debug", console_log_color=True,
                        ...
                        logfile_log_datefmt='%Y/%m/%d %I:%M:%S %p', log_line_template="%(color_on)s[%(created)d] [%(threadName)s] [%(levelname)-8s] %(message)s%(color_off)s")):
        print("Failed to setup logging, aborting.")
        return 1

    logger = logging.getLogger()
    logger.debug("Logging intialized")
