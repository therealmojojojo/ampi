...
try:
    ...
    logger.info("2.Drawing on the Vertical image...")
    ...
    logger.info("3.read bmp file")
    Himage = Image.open(os.path.join(picdir, '2in9.bmp'))
    epd.display(epd.getbuffer(Himage))
    time.sleep(2)
    
    logger.info("4.read bmp file on window")
